...
public class UserRepository extends MongoRepository {
	...
	public User getUserByEmail(String sUserEmail) {
		...
		if (Utils.isNullOrEmpty(sUserEmail))
			return null;
	...
	public List<User> getAdminsOfOrganization(String sOrganizationId) {
		...
		try {
			...
			FindIterable<Document> oWSDocument = getCollection(m_sThisCollection).find(Filters.and(
					Filters.eq("organizationId", sOrganizationId), Filters.or(Filters.eq("role", UserRole.ADMIN.name()),
							Filters.eq("role", UserRole.RISE_ADMIN.name()))));
	...
	public List<User> getFieldOperatorsOfOrganization(String sOrganizationId) {
		...
		try {
			...
			FindIterable<Document> oWSDocument = getCollection(m_sThisCollection).find(
					Filters.and(Filters.eq("organizationId", sOrganizationId), Filters.eq("role", UserRole.FIELD.name())));
			...
			RiseLog.errorLog("UserRepository.getFieldOperatorsOfOrganization: error", oEx);
		}
	...
	public List<User> getHQOperatorsOfOrganization(String sOrganizationId) {
		...
		try {
			...
			FindIterable<Document> oWSDocument = getCollection(m_sThisCollection).find(
					Filters.and(Filters.eq("organizationId", sOrganizationId), Filters.eq("role", UserRole.HQ.name())));
