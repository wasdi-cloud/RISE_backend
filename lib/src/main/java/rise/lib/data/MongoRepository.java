...
public class MongoRepository {
    ...
    public static void readConfig() {
        MongoRepository.SERVER_ADDRESS = RiseConfig.Current.mongoMain.address;
        MongoRepository.DB_NAME = RiseConfig.Current.mongoMain.dbName;
        MongoRepository.DB_USER = RiseConfig.Current.mongoMain.user;
        MongoRepository.DB_PWD = RiseConfig.Current.mongoMain.password;
        MongoRepository.REPLICA_NAME = RiseConfig.Current.mongoMain.replicaName;
    }
    ...
    public static void addMongoConnection(String sDbCode, String sUser, String sPassword, String sAddress, String sReplicaName, String sDbName) {
    	try 
    	{
    		...
    		if (oMongoConnection == null) {
            	...
            	RiseLog.debugLog("MongoRepository.addMongoConnection: Configuration added: " + sDbCode);
    		}
    	}
    	catch (Exception e) 
    	{
    		RiseLog.errorLog("MongoRepository.addMongoConnection: exception " + e.getMessage());
    	}    	
    }
    ...
    public static MongoDatabase getMongoDatabase(String sDbCode) {
    		...
    		if (oMongoConnection == null) {
    			...
    			if (sDbCode.equals("rise")) {
    				...
    				if (!s_bDbSwitchLogged) {
    					RiseLog.debugLog("MongoRepository.getMongoDatabase: Db Code " + sDbCode + " NOT FOUND. try to recover with default db");
    	...
    	{
    		RiseLog.errorLog("MongoRepository.getMongoDatabase: exception " + e.getMessage());
    	}
        return null;
    }
    	...
    	try {
    		...
    		for (MongoConnection oConnection : aoConnections) {
        		try {
    				...
    				RiseLog.errorLog("MongoRepository.shutDownConnection: exception " + e.getMessage());
    			}
			}
    	}
    	catch (Exception e) {
        ...
        try {
        	while (oCursor.hasNext()) {
                ...
                try {
                	...
                	RiseLog.errorLog(m_sThisCollection + ".fillList: " + oEx);
                }            		
        	}
        }
        finally {
	...
	protected int delete(BasicDBObject oCriteria, String sCollectionName ){
        try {
        	...
        	RiseLog.errorLog("MongoRepository.delete: excetpion " + oEx.toString());
        }
	...
	protected int deleteMany(BasicDBObject oCriteria, String sCollectionName){
        try {
        	...
        	RiseLog.errorLog("MongoRepository.deleteMany: excetpion " + oEx.toString());
        }
