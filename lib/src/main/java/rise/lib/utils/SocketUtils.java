...
public class SocketUtils {
	...
	private static DockerHttpClient initializeDockerClient(String sDockerHostAddress) {
		...
		try {
			...
			RiseLog.errorLog("SocketUtils.initializeDockerClient. Error in the initialization of the Docker HTTP client." + oEx.getMessage());
		}
	...
	private static DockerClientConfig getDockerClientConfig() {
		String sDockerHost = RiseConfig.Current.dockers.internalDockerAPIAddress;
	...
	private static DockerHttpClient getDockerHttpClient(DockerClientConfig oDockerClientConfig) {
		DockerHttpClient oHttpClient = new ApacheDockerHttpClient.Builder()
			    ...
			    .connectionTimeout(Duration.ofMillis(RiseConfig.Current.connectionTimeout))
			    .build();
	...
	public static void copyHeaders(Map<String, List<String>> aoReceivedHeaders, Map<String, List<String>> aoOutputHeaders) {
		try {		
			...
			RiseLog.errorLog("SocketUtils.copyHeaders: Exception getting the output headers ", oEx);
		}
	}
	...
	public static HttpCallResponse httpGet(String sPath, Map<String, String> asHeaders, Map<String, List<String>> aoOutputHeaders) {
		...
		if (Utils.isNullOrEmpty(sPath)) {
			RiseLog.errorLog("SocketUtils.httpGet: No Docker enpoint specified. Returning an empty result.");
		...
		if (oHttpClient == null) {
			RiseLog.errorLog("SocketUtils.httpGet. Docker HTTP client is null. Returning an empty response.");
			return oHttpCallResponse;
		}
		
		String sBaseAddress = RiseConfig.Current.dockers.internalDockerAPIAddress;
		...
		try (Response oResponse = oHttpClient.execute(oRequest)) {
			...
			if (RiseConfig.Current.logHttpCalls || (iResponseCode<200||iResponseCode>299)) {
				RiseLog.debugLog("SocketUtils.httpGet: Response code " + iResponseCode);
			}
			...
			RiseLog.errorLog("SocketUtils.httpGet: Exception ", oEx);
		} finally {
			try {
				oHttpClient.close();
			} catch (IOException oEx) {
				RiseLog.errorLog("SocketUtils.httpGet: Impossible to close the connection ", oEx);
			}
		}
	...
	public static HttpCallResponse httpPost(String sPath, File oFile, Map<String, String> asHeaders) {
		...
		if (Utils.isNullOrEmpty(sPath)) {
			RiseLog.errorLog("SocketUtils.httpPost: No Docker enpoint specified. Returning an empty result");
			return oHttpCallResponse;
		}
		...
		if (oHttpClient == null) {
			RiseLog.errorLog("SocketUtils.httpPost. Docker HTTP client is null. Returning an empty response.");
			return oHttpCallResponse;
		}
		...
		// RiseLog.debugLog("SocketUtils.httpPost: computed path " + sPath);
		...
		try {
			...
			RiseLog.errorLog("SocketUtils.httpPost. Error providing the input stream. Returning an empty response.");
			return oHttpCallResponse;
		}
		...
		try (Response oResponse = oHttpClient.execute(oRequest)) {
			...
			RiseLog.debugLog("SocketUtils.httpPost: Exception " + oEx.getMessage());
		} finally {
			try {
				oHttpClient.close();
			} catch (IOException oEx) {
				RiseLog.errorLog("SocketUtils.httpPost: Impossible to close the connection ", oEx);
			}
		}
		return oHttpCallResponse;		
	}
	...
	public static HttpCallResponse httpDelete(String sPath, Map<String, String> asHeaders) {
		...
		if (Utils.isNullOrEmpty(sPath)) {
			RiseLog.errorLog("SocketUtils.httpDelete: No Docker enpoint specified. Returning an empty result");
			return oHttpCallResponse;
		}
		...
		if (oHttpClient == null) {
			RiseLog.errorLog("SocketUtils.httpDelete. Docker HTTP client is null. Returning an empty response.");
			return oHttpCallResponse;
		}	
		...
		try (Response oResponse = oHttpClient.execute(oRequest)) {
			...
			RiseLog.debugLog("SocketUtils.httpDelete: Exception " + oEx.getMessage());
		} finally {
			try {
				oHttpClient.close();
			} catch (IOException oEx) {
				RiseLog.errorLog("SocketUtils.httpDelete: Impossible to close the connection " + oEx);
			}
		}
	...
	public static HttpCallResponse httpPut(String sPath, Map<String, String> asHeaders, byte[] ayBytes) {
		...
		if (Utils.isNullOrEmpty(sPath)) {
			RiseLog.errorLog("SocketUtils.httpPut: No Docker enpoint specified. Returning an empty result");
			return oHttpCallResponse;
		}
		...
		if (oHttpClient == null) {
			RiseLog.errorLog("SocketUtils.httpPut. Docker HTTP client is null. Returning an empty response.");
			return oHttpCallResponse;
		}	
		...
		try (Response oResponse = oHttpClient.execute(oRequest)) {
			...
			RiseLog.errorLog("SocketUtils.httpPut: Exception ", oEx);
		} finally {
			try {
				oHttpClient.close();
			} catch (IOException oEx) {
				RiseLog.errorLog("SocketUtils.httpPut: Impossible to close the connection ", oEx);
			}
		}
	...
	public static HttpCallResponse httpPostFile(String sPath, String sFileName, Map<String, String> asHeaders) {
		...
		if (!oFile.exists()) {
			RiseLog.errorLog("SocketUtils.httpPostFile: file not found.");
		...
		try {
			...
			if (oHttpClient == null) {
				RiseLog.errorLog("SocketUtils.httpPostFile. Docker HTTP client is null. Returning an empty response.");
				return oHttpResponse;
			}	
			...
			try (InputStream oFileInStream = new FileInputStream(oFile)) {
				...
				try (Response oResponse = oHttpClient.execute(oRequest)) {
					...
					RiseLog.errorLog("SocketUtils.httpPostFile: Exception when trying to execute the request", oEx);
				} 	
			}
			
		} catch (Exception oE) {
			RiseLog.debugLog("SocketUtils.httpPostFile: could not open file due to: " + oE.getMessage() + ", aborting");
		} finally {
			try {
