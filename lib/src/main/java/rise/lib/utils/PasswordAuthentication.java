...
public final class PasswordAuthentication {
	...
	public boolean isValidPassword(String sPassword) {
		...
		if (Utils.isNullOrEmpty(sPassword))
			return false;
		if (sPassword.length() < RiseConfig.Current.security.minPwLenght)
			return false;

		if (RiseConfig.Current.security.mustContainNumber) {
			if (!sPassword.matches(".*\\d.*"))
				return false;
		}
		...
		if (RiseConfig.Current.security.mustMixUpperLowerCase) {
			...
			if (!(oHasLower.find() && oHasUpper.find()))
				return false;
		}
	...
	public boolean isValidUserId(String sUserId) {
		if (Utils.isNullOrEmpty(sUserId))
			return false;
		// must be longer than 8 char
		if (sUserId.length() < RiseConfig.Current.security.minUserIdLenght)
			return false;
		// must be all lower case
		if (!sUserId.toLowerCase().equals(sUserId))
			return false;
